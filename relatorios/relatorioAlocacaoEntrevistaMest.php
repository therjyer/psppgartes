...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        //t�tulo do relat�rio
        $titulo = "PROGRAMA DE P�S-GRADUAÇÃO EM ARTES";
        $pdf->setXY(1.5, 1);
        ...
        //per�odo da busca
        $periodo = " PROCESSO SELETIVO " . $processo . " - RELATÓRIO DE ALOCAÇÃO PARA AVALIAÇÃO DE PROVA ORAL (MESTRADO)";
        $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf, c.tipoProcesso, c.notaProva1, c.notaProva2, c.pontuacaoCurriculo, c.avaliadorEntrevista1, c.avaliadorEntrevista2 FROM candidato c WHERE c.processo = $idProcesso AND c.estado=1 AND c.estadoHomologacao = 1 AND c.tipoProcesso=1 AND (pontuacaoCurriculo >=7) ORDER BY c.avaliadorEntrevista1, c.nome");
...
//for ($i = 0; $i < $total_results; $i++) {
    ...
    //$nome = mysql_result($result, $i, 'nome');     $cpf = mysql_result($result, $i, 'cpf');

    $cpfLimpo = remove($cpf);
    $cpfFormatado = mascara($cpfLimpo, '###.###.###-##');

    $idProfessor1 = $row['avaliadorEntrevista1']; //    $idProfessor1 = mysql_result($result, $i, 'avaliadorEntrevista1');
    ...
    if ($idProfessor1 == "-") {
        ...
        $rowprofessor1 = mysqli_fetch_row($consulta3);
    ...
    if ($idProfessor2 == "-") {
        ...
        $rowprofessor2 = mysqli_fetch_row($consulta2);
...
$pdf->Output("Relat�rio de Alocação de Entrevista Mestrado(Processo Seletivo " . $processo . ").pdf", "I");
?>