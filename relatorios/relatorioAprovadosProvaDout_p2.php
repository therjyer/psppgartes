...
//$processo = $_GET['processo']; 
        ...
        //período da busca
        $periodo = " PROCESSO SELETIVO " . $processo . " - LISTA DE APROVADOS NA PROVA (DOUTORADO)";
        $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaProva1, c.notaProva2 FROM candidato c WHERE c.cotas = 'ac' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaProva1+c.notaProva2)/2)>=7) AND c.tipoProcesso=2 ORDER BY c.linhaPesquisa");
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaProva1, c.notaProva2 FROM candidato c WHERE ( c.cotas = 'er' OR c.idCandidato = 779 ) AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaProva1+c.notaProva2)/2)>=7)  AND c.tipoProcesso=2 ORDER BY c.linhaPesquisa");
$total_results = mysqli_num_rows($result);
...
//IES - Amazonia Legal e Região Nordeste
...
$pdf->Ln();
       ...
       $pdf->setX(1.7);
        ...
        $pdf->Cell(17.5, 0.6, 'IES - Amazonia Legal e Região Nordeste', 1, 1, "C");
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaProva1, c.notaProva2 FROM candidato c WHERE c.cotas = 'padt' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaProva1+c.notaProva2)/2)>=7) AND c.tipoProcesso=2 ORDER BY c.linhaPesquisa");
$total_results = mysqli_num_rows($result);
...
$info = "No valor da nota de cada cantidato foi aplicado o peso 2";
$pdf->setX(1.7);
$pdf->SetFont('arial', '', 10);
$pdf->MultiCell(18, 0.5, $info, 0, 'L');
//$pdf->Ln();
//função para exibir o relatório gerado em um arquivo .pdf no navegador
$pdf->Output("relatorioAprovadosProvaDout_p2.pdf", "I");
?>
