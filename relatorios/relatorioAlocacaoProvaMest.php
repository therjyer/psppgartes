...
class PDF extends FPDF
{
 function Header()
 {
      ...
      //período da busca
      $periodo= " PROCESSO SELETIVO ".$processo." - RELATÓRIO DE ALOCAÇÃO PARA PROVA (MESTRADO)";
      $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf, c.tipoProcesso, c.notaAnteprojeto1, c.notaAnteprojeto2, c.avaliadorProva1, c.avaliadorProva2 FROM candidato c WHERE c.processo = $idProcesso AND c.estado=1 AND c.estadoHomologacao = 1 AND c.tipoProcesso=1 AND ((c.notaAnteprojeto1+c.notaAnteprojeto2)/2)>=7 AND `avaliadorProva1` <> '' AND `avaliadorProva2` <> '' ORDER BY c.avaliadorProva1, c.nome");
