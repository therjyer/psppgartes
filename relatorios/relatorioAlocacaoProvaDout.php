...
class PDF extends FPDF
{
 function Header()
 {
      ...
      //período da busca
      $periodo= " PROCESSO SELETIVO ".$processo." - RELATÓRIO DE ALOCAÇÃO PARA PROVA (DOUTORADO)";
      $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf, c.tipoProcesso, c.notaAnteprojeto1, c.notaAnteprojeto2, c.avaliadorProva1, c.avaliadorProva2 FROM candidato c WHERE c.processo = $idProcesso AND c.estado=1 AND c.estadoHomologacao = 1 AND c.tipoProcesso=2 AND ((c.notaAnteprojeto1+c.notaAnteprojeto2)/2)>=7 AND `avaliadorProva1` <> '' AND `avaliadorProva2` <> '' ORDER BY c.avaliadorProva1, c.nome");
...
for ($i=0;$i<$total_results;$i++){
	...
	$cpfFormatado = mascara($cpfLimpo,'###.###.###-##');
        ...
        $idProfessor1 = mysql_result($result, $i, 'avaliadorProva1');       
 if (($idProfessor1 == "") OR  ($idProfessor1 == "-") ) {
        $nomeProfessor1 = "-";
    } else {
    ...
    $idProfessor2 = mysql_result($result, $i, 'avaliadorProva2');
 if ($idProfessor2 == "") {
        $nomeProfessor2 = "-";
    } else {

        $consulta3 = mysqli_query($_SG['link'], "SELECT u.nome FROM usuarios u WHERE u.id=$idProfessor2");
        $nomeProfessor2 = mysql_result($consulta3, 0, 'nome');
    }
