...
//ETNORACIAL  BEGIN ############################################################################
    ...
    $pdf->Cell(27.4, 0.6, 'ETNORACIAL', 1, 1, "C");
...
// $pdf->Cell(2.7,0.6,'Média',1,0,"C");
    ...
    $result = mysqli_query($_SG['link'], "SELECT  c.idCandidato, c.nome, c.linhaPesquisa,  ((c.notaAnteprojeto1+c.notaAnteprojeto2)/2) AS notaAnteprojeto, c.pontuacaoCurriculo, c.notaEntrevista , c.notaFinal, ((((c.notaAnteprojeto1+c.notaAnteprojeto2)/2) + c.pontuacaoCurriculo + c.notaEntrevista) / 3 ) AS notaFinalCalc, c.colocacao AS colocacao, c.resultadoFinal AS resultadoFinal, `u1`.`nome` AS `orientador` FROM candidato c left join `usuarios` `u1` on(`c`.`optOrientador1` = `u1`.`id`) WHERE  c.processo = $idProcesso AND c.estadoEntrevista = 1 AND c.estado=1 AND c.tipoProcesso = $idTipoProcesso AND c.estadoCurriculo = 1 AND c.cotas in ('pcd') AND c.notaEntrevista >= 7 ORDER BY c.linhaPesquisa, c.colocacao");  //'cc','cct'
...
//
    ...
    $pdf->Cell(27.4, 0.6, 'COTA TRANS', 1, 1, "C");
...
// $pdf->Cell(2.7,0.6,'Média',1,0,"C");
    ...
    $result = mysqli_query($_SG['link'], "SELECT  c.idCandidato, c.nome, c.linhaPesquisa,  ((c.notaAnteprojeto1+c.notaAnteprojeto2)/2) AS notaAnteprojeto, c.pontuacaoCurriculo, c.notaEntrevista , c.notaFinal, ((((c.notaAnteprojeto1+c.notaAnteprojeto2)/2) + c.pontuacaoCurriculo + c.notaEntrevista) / 3 ) AS notaFinalCalc, c.colocacao AS colocacao, c.resultadoFinal AS resultadoFinal, `u1`.`nome` AS `orientador` FROM candidato c left join `usuarios` `u1` on(`c`.`optOrientador1` = `u1`.`id`) WHERE  c.processo = $idProcesso AND c.estadoEntrevista = 1 AND c.estado=1 AND c.tipoProcesso = $idTipoProcesso AND c.estadoCurriculo = 1 AND c.cotas in ('ct') AND c.notaEntrevista >= 7 ORDER BY c.linhaPesquisa, c.colocacao");
