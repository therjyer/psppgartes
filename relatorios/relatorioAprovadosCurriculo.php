...
//$processo = $_GET['processo']; 
        ...
        if ($idTipoProcesso == "1")
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - LISTA DE CLASSIFICADOS NA ANÁLISE DE CURRÍCULO (MESTRADO)";
            } else
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - LISTA DE CLASSIFICADOS NA ANÁLISE DE CURRÍCULO (DOUTORADO)";
            }
...
//AMPLA COMCORRÊNCIA
    ...
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas = 'ac' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso= $idTipoProcesso ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
    $pdf->SetFont('arial', '', 9);
    while ($row = mysqli_fetch_array($result)) {
        ...
        $nota = number_format($row['pontuacaoCurriculo'], 2, '.', '');
        if ($linhaPesquisa == "1")
            {
            $linha = "Linha 1";
...
$pdf->Ln();
    ...
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas in ('er') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=$idTipoProcesso ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
...
$pdf->Ln();
    ...
    $pdf->Cell(4, 0.6, 'Linha de Pesquisa', 1, 1, "C");
    //$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaAnteprojeto1, c.notaAnteprojeto2 FROM candidato c WHERE c.cotas in ('cc','cct') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaAnteprojeto1+c.notaAnteprojeto2)/2)>=7) AND c.tipoProcesso= $idTipoProcesso ORDER BY c.linhaPesquisa, c.nome"); 
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas in ('pcd') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=$idTipoProcesso ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
//$total_results = mysqli_num_rows($result);
    $pdf->SetFont('arial', '', 9);
    while ($row = mysqli_fetch_array($result)) {
        $nome = $row['nome'];
...
$pdf->Ln();
    ...
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas in ('ct') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=$idTipoProcesso ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
//$total_results = mysqli_num_rows($result);
    $pdf->SetFont('arial', '', 9);
    while ($row = mysqli_fetch_array($result)) {
        $nome = $row['nome'];
...
$pdf->Ln();
    ...
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas in ('padt', 'padtcc') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=$idTipoProcesso ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
//$total_results = mysqli_num_rows($result);
    $pdf->SetFont('arial', '', 9);
    while ($row = mysqli_fetch_array($result)) {
        $nome = $row['nome'];
...
//$pdf->Output("relatorioAprovadosProjeto.pdf", "I");
        if ($idTipoProcesso == "1")
            {
            $pdf->Output("relatorioAprovadosCurriculo_Mestrado.pdf", "I");
            } else
            {
            $pdf->Output("relatorioAprovadosCurriculo_Doutorado.pdf", "I");
            }
