...
//$processo = $_GET['processo']; 
        ...
        $idTipoProcesso = $_GET['idTipo'];
        if ($idTipoProcesso == "1") {
            $periodo = " PROCESSO SELETIVO " . $processo . " - CANDIDATOS E NOTAS DA ENTREVISTA (MESTRADO)";
        } else {
            $periodo = " PROCESSO SELETIVO " . $processo . " - CANDIDATOS E NOTAS DA ENTREVISTA (DOUTORADO)";
        }
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT * FROM `viewnotasentrevista` WHERE PROCESSO = $idProcesso AND tipoProcesso = $idTipoProcesso");
...
if ($idTipoProcesso == "1") {
    $pdf->Output("relatorioNotasEntrevistaMestrado.pdf", "I");
} else {
    $pdf->Output("relatorioNotasEntrevistaDoutorado.pdf", "I");
}
?>
