...
$pdf->Ln();
    ...
    $pdf->Cell(4, 0.6, 'Nota', 1, 0, "C");
    $pdf->Cell(4, 0.6, 'Linha de Pesquisa', 1, 1, "C");
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaAnteprojeto1, c.notaAnteprojeto2 FROM candidato c WHERE c.cotas in ('er') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaAnteprojeto1+c.notaAnteprojeto2)/2)>=7) AND c.tipoProcesso= $idTipoProcesso ORDER BY c.linhaPesquisa, c.nome"); 
...
$pdf->Ln();
    ...
    $pdf->Cell(4, 0.6, 'Nota', 1, 0, "C");
    $pdf->Cell(4, 0.6, 'Linha de Pesquisa', 1, 1, "C");
    $result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaAnteprojeto1, c.notaAnteprojeto2 FROM candidato c WHERE c.cotas in ('pcd') AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaAnteprojeto1+c.notaAnteprojeto2)/2)>=7) AND c.tipoProcesso= $idTipoProcesso ORDER BY c.linhaPesquisa, c.nome"); 
//$total_results = mysqli_num_rows($result);
    $pdf->SetFont('arial', '', 9);
    while ($row = mysqli_fetch_array($result)) {
        $nome = $row['nome'];
