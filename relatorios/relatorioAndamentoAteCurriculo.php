...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $pdf->MultiCell(26, 0.8, $titulo, 0, 'C');
        $pdf->Ln();

        $pdf->SetFillColor(0, 0, 0);
        $pdf->setXY(1.5, 1.7);
        $pdf->Cell(26, 0.1, ' ', 1, 1, "C");
        $pdf->setXY(1.5, 1.8);
        $pdf->Cell(26, 0.1, ' ', 1, 1, "C", true);
        ...
        if ($idTipoProcesso == "1")
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - RELATÓRIO DE NOTAS ATÉ ETAPA CURRÍCULO (MESTRADO)";
            } else
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - RELATÓRIO DE NOTAS ATÉ ETAPA CURRÍCULO (DOUTORADO)";
            }


        $pdf->setXY(1.5, 2);
        $pdf->SetFont('arial', '', 10);
        $pdf->MultiCell(26, 0.8, $periodo, 0, 'C');
        $pdf->Ln();

        //cabe�alho da tabela
        $pdf->SetFont('arial', 'B', 9);
        $pdf->setXY(1.7, 4);
        $pdf->Cell(10, 0.6, 'Nome do candidato', 1, 0, "C");
        $pdf->Cell(6, 0.6, 'Orientador', 1, 0, "C");
        //$pdf->Cell(5.3, 0.6, 'Orientador2', 1, 0, "C");
        $pdf->Cell(2.2, 0.6, 'Linha', 1, 0, "C");
        $pdf->Cell(2.5, 0.6, 'Área', 1, 0, "C");
        $pdf->Cell(2, 0.6, 'Nota Proj', 1, 0, "C");
        $pdf->Cell(2, 0.6, 'Nota Curr', 1, 1, "C");
    }
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT * FROM `viewandamentoatecurriculo` WHERE processo = $idProcesso and tipoProcesso = $idTipoProcesso");
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $pdf->setX(1.7);
    $pdf->Cell(10, 0.6, $nome, 1, 0, "L");
    $pdf->Cell(6, 0.6, $nomeProfessor1, 1, 0, "L");
    //$pdf->Cell(5.3, 0.6, $nomeProfessor2, 1, 0, "C");
    $pdf->Cell(2.2, 0.6, $linhaPesquisa, 1, 0, "C");
    $pdf->Cell(2.5, 0.6, $area, 1, 0, "C");
    $pdf->Cell(2, 0.6, $nota, 1, 0, "C");
    $pdf->Cell(2, 0.6, $notaCurriculo, 1, 1, "C");
}
...
if ($idTipoProcesso == "1")
    {
    $pdf->Output("relatorioAndamentoAteCurriculo_Mestrado.pdf", "I");
    } else
    {
    $pdf->Output("relatorioAndamentoAteCurriculo_Doutorado.pdf", "I");
    }
?>