...
//$processo = $_GET['processo']; 
        ...
        //período da busca
        $periodo = " PROCESSO SELETIVO " . $processo . " - LISTA DE CLASSIFICADOS NA ANÁLISE DE CURRÍCULO(DOUTURADO)";
        $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas = 'ac' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=2 ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE ( c.cotas = 'er' OR c.idCandidato = 779 ) AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=2 ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
$total_results = mysqli_num_rows($result);
...
$pdf->Cell(17.5, 0.6, 'ES - Amazonia Legal e Região Nordeste', 1, 1, "C");
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas = 'padt' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=2 ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
$total_results = mysqli_num_rows($result);
...
$pdf->Output("relatorioAprovadosCurrDout.pdf", "I");
?>
