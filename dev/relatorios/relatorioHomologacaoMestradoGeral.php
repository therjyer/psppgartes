...
class PDF extends FPDF
{
 function Header()
 {
     ...
     // $processo = $_GET['processo']; 
      ...
      //per�odo da busca
      $periodo= " PROCESSO SELETIVO ".$processo." - RELATÓRIO DE HOMOLOGAÇÃO (MESTRADO)";
      $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT DISTINCT c.nome, c.numInscricao, c.cpf, c.estadoHomologacao from candidato c WHERE c.processo = $idProcesso AND c.estado=1 AND c.tipoProcesso = 1 ORDER BY c.nome");
...
$pdf->Output("RELATÓRIO GERAL DE HOMOLOGAÇÃO MESTRADO (Processo Seletivo ".$processo.").pdf","I");
