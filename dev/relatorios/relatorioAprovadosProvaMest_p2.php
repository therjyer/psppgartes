...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaProva1, c.notaProva2 FROM candidato c WHERE c.cotas = 'ac' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaProva1+c.notaProva2)/2)>=7) AND c.tipoProcesso=1 ORDER BY c.linhaPesquisa");
...
$pdf->Ln();
        ...
        $pdf->SetFont('arial', 'B', 9);
        $pdf->setX(1.7);
        $pdf->Cell(17.5, 0.6, 'ETNORRACIAL', 1, 1, "C");
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.notaProva1, c.notaProva2 FROM candidato c WHERE c.cotas = 'er' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (((c.notaProva1+c.notaProva2)/2)>=7) AND c.tipoProcesso=1 ORDER BY c.linhaPesquisa");
$total_results = mysqli_num_rows($result);
...
$pdf->Output("relatorioAprovadosProvaMest.pdf", "I");
?>
