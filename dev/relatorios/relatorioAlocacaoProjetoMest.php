...
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf, c.tipoProcesso, c.notaProva1, c.notaProva2, c.optOrientador1, c.optOrientador2 FROM candidato c WHERE c.processo = $idProcesso AND c.estado=1 AND c.estadoHomologacao = 1 AND c.tipoProcesso=1 ORDER BY c.optOrientador1, c.nome");
...
while ($row = mysqli_fetch_array($result) ) {
    ...
    $cpf = $row['cpf'];
    $idProfessor1 = $row['optOrientador1'];
    if ($idProfessor1 == "") {
        $nomeProfessor1 = "-";
    } else {
    ...
    $idProfessor2 = $row['optOrientador2'];
