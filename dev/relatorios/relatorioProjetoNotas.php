...
//$processo = $_GET['processo']; 
        ...
        if ($idTipoProcesso == "1")
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - CANDIDATOS E NOTAS DO PROJETO (MESTRADO)";
            } else
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - CANDIDATOS E NOTAS DO PROJETO (DOUTORADO)";
            }
        ...
        $pdf->Cell(3, 0.6, 'Nota Avaliador 1', 1, 0, "C");
        $pdf->Cell(3, 0.6, 'Nota Avaliador 2', 1, 1, "C");
    }
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT * FROM `viewnotasprojeto` WHERE PROCESSO = $idProcesso AND tipoProcesso = $idTipoProcesso");
...
while ($row = mysqli_fetch_array($result) ) {
    ...
    $pdf->Cell(3, 0.6, number_format($nota2, 2, ',', ' '), 1, 1, "C");
