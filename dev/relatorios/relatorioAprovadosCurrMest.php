...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas = 'ac' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=1 ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf,c.linhaPesquisa, c.pontuacaoCurriculo FROM candidato c WHERE c.cotas = 'er' AND c.processo = $idProcesso AND c.estadoHomologacao=1 AND c.estado=1 AND (c.pontuacaoCurriculo >=7) AND c.tipoProcesso=1 ORDER BY  c.linhaPesquisa, c.pontuacaoCurriculo DESC");
$total_results = mysqli_num_rows($result);
...
$pdf->Output("relatorioAprovadosCurrMest.pdf", "I");
?>
