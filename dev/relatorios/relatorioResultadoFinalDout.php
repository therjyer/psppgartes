...
$linha3Tc = 2;  // 1 vaga + 1 doação de padt
*/
...
$result = mysqli_query($_SG['link'], "SELECT  c.idCandidato, c.nome, c.linhaPesquisa,  ((c.notaAnteprojeto1+c.notaAnteprojeto2)/2) AS notaAnteprojeto, ((c.notaProva1+c.notaProva2)) AS notaProva , c.pontuacaoCurriculo, c.notaEntrevista , c.notaFinal, CONVERT((((c.notaAnteprojeto1+c.notaAnteprojeto2)/2) + ((c.notaProva1+c.notaProva2)) + c.pontuacaoCurriculo + c.notaEntrevista), DECIMAL(10,2)) AS notaFinalCalc
...
//    if ($i == 15) {
//      $pdf->setX(1); 
//      $pdf->Cell(27.2, 0.6, 'Do 9º ao 10º LINHAS 02 OU 03, CONFORME ADENDO PUBLICADO EM 13/06/2018', 1, 1, "C");  
//    }
...
//    }
    ...
    $resultadoFinal = 0; 
    switch ($resultado) {
    case "Aprovado e classificado":
        $resultadoFinal =1;
        break;
        case "Aprovado, mas não classificado":
        $resultadoFinal =2;
        break;
    case "Reprovado":
         $resultadoFinal =3;
    ...
    $query = "UPDATE candidato SET notaFinal='$notaFinal', resultadoFinal='$resultadoFinal'  WHERE idCandidato = $idCandidato";
...
$pdf->SetFont('arial', 'B', 9);
$pdf->setX(1);   //$pdf->setXY(1, 3.4);
$pdf->Cell(27.2, 0.6, 'IES - Amazonia Legal e Região Nordeste', 1, 1, "C");
...
$pdf->Output("Relatório Geral - DOUTORADO (Processo Seletivo " . $processo . ").pdf", "I");
?>