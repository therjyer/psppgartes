...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        //período da busca
        $periodo = " PROCESSO SELETIVO " . $processo . " - RELATÓRIO DE ALOCAÇÃO PARA AVALIAÇÃO DE PROJETO (DOUTORADO)";
        $pdf->setXY(1.5, 2);
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT c.nome, c.numInscricao, c.cpf, c.tipoProcesso, c.notaProva1, c.notaProva2, c.avaliadorAnteprojeto1, c.avaliadorAnteprojeto2 FROM candidato c WHERE c.processo = $idProcesso AND c.estado=1 AND c.estadoHomologacao = 1 AND c.tipoProcesso=2 ORDER BY c.avaliadorAnteprojeto1, c.nome");
...
for ($i = 0; $i < $total_results; $i++) {
    ...
    $idProfessor1 = mysql_result($result, $i, 'avaliadorAnteprojeto1');
    ...
    $idProfessor2 = mysql_result($result, $i, 'avaliadorAnteprojeto2');
    if ($idProfessor2 == "") {
        $nomeProfessor2 = "-";
    } else {
