...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        if ($idTipoProcesso == "1")
            ...
            {
            $periodo = " PROCESSO SELETIVO " . $processo . " - RELATÓRIO DE ALOCAÇÃO PARA AVALIAÇÃO DE CURRÍCULO (DOUTORADO)";
            }
        ...
        $pdf->Cell(5.5, 0.6, 'Avaliador', 1, 0, "C");
        $pdf->Cell(2.5, 0.6, 'Linha', 1, 0, "C");
        $pdf->Cell(3, 0.6, 'Área', 1, 0, "C");
        $pdf->Cell(3, 0.6, 'Nota Projeto', 1, 1, "C");
    }
...
//tabela de dados
$result = mysqli_query($_SG['link'], "SELECT * FROM `viewalocavalidacurriculo` WHERE processo = $idProcesso and tipoProcesso = $idTipoProcesso");
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $pdf->Cell(3, 0.6, $cpfFormatado, 1, 0, "C");
    $pdf->Cell(5.5, 0.6, $nomeProfessor1, 1, 0, "C");
    $pdf->Cell(2.5, 0.6, $linhaPesquisa, 1, 0, "C");
    $pdf->Cell(3, 0.6, $area, 1, 0, "C");
    $pdf->Cell(3, 0.6, $nota, 1, 1, "C");
}
...
if ($idTipoProcesso == "1")
    {
    $pdf->Output("relatorioAlocacaoAvaliadorCurriculo_Mestrado.pdf", "I");
    } else
    {
    $pdf->Output("relatorioAlocacaoAvaliadorCurriculo_Doutorado.pdf", "I");
    }
?>