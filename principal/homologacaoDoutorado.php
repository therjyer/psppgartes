                    ...
                    <section class="clearfix">
                        <div class="g3">
                            <div>
                               <div class="info">
                                <h4>
                                    Selecione um canditado para homologar (Doutorado)
                                </h4>
                               </div>
                                <div align="center">
                                    <form class="form-wrapper cf" method="post" action="homologacaoDout.php?idProcesso=<?php echo $idProcesso; ?>">
...
include("dbconnect.inc.php");
	...
	$result = mysqli_query($_SG['link'], "SELECT c.estado, c.estadoHomologacao, c.idCandidato, c.numInscricao, c.nome, c.cpf FROM candidato c WHERE  (c.nome like '%$busca%' OR c.cpf like '%$busca%' OR c.numInscricao like '%$busca%') AND c.estado = 1 AND c.processo = '$idProcesso' AND c.tipoProcesso = 2 ORDER BY c.nome")
		or die(mysql_error());
	...
	$total_pages = ceil($total_results / $per_page);
        ...
        if(isset($_GET['page'])){
            ...
            for ($i = 1; $i <= $total_pages; $i++)
           {
            if($pg_atual > 0){
               if  ($i==$pg_atual){
                   echo "<li ><a class='current' href='homologacaoDoutorado.php?page=$i&idProcesso=$idProcesso'> $i </a></li>";
                } else{
                    echo "<li><a href='homologacaoDoutorado.php?page=$i&idProcesso=$idProcesso'> $i </a></li>";
               }
               }else{
                    echo "<li ><a class='current' href='homologacaoDoutorado.php?page=1&idProcesso=$idProcesso'> 1 </a></li>";
                     $pg_atual=1;
               }
            }
	...
	//for ($i = $start; $i < $end; $i++)
        while ($row = mysqli_fetch_array($result) )
        {
               ...
               // antigo if ($i == $total_results) { break; }
				...
				echo "<td> <a href='homologarDoutorado.php?estadoHomologacao=1&idCandidato=$idCandidato&idProcesso=$idProcesso&page=$pg'>Sim</a> / <a href='homologarDoutorado.php?estadoHomologacao=2&idCandidato=$idCandidato&idProcesso=$idProcesso&page=$pg'>Não</a></td>";                                
		                echo "</tr>";
        }   ?>
